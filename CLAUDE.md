...
`~`（補集）> `> W:n` / `< H:n`（grow/shrink，單位 nm）> `&`（AND）> `|` / `-`（OR / 差集）。
輸出 shapely polygon（canvas 顯示）+ uint8 mask（量測）。
...
- [F4] Boolean 表達式引擎強化：食譜化重算 + 巢狀 + 編輯 + 對話框重設計 —
  `glas/core/gds_boolean.py` / `glas/app/gds_align_tool.py` —
  plan: `docs/plans/F4-boolean-enhance.md`（3 milestone，待核准）
- [F5] Fine align 診斷：殘差疊圖 + 批次結果總覽 —
  `glas/app/gds_align_tool.py` — plan: `docs/plans/F5-finealign-diagnostics.md`（2 milestone，排 F4 之後）

### 待辦 (Backlog)

- [F1] 互動驗收：真實 SEM↔GDS 對位 / 拖動 δ / fine-align score / 批次 Run all /
  匯出 / 折疊 UX（自 MMH 抽離前即標記「待 user 本地驗證」的項目，移到 GLAS 接續） — `glas/app/gds_align_tool.py` — 高優先
