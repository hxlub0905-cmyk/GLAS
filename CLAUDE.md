...
- [F11] 整顆 chip OASIS 匯出（原始 + Boolean 全 chip 重算）+ GDS 座標可見性
  → M2/M3 已實作（tiled streaming worker + 匯出對話框 scope 選項 + 0–100% 進度條/ETA/
  取消）。注意：含千萬級 placement 的 mega-cell 檔（[F14]）會逐 tile 極慢。見
  `docs/plans/F11-whole-chip-export.md`
- [F13] 讀 KLayout per-cell S_BOUNDING_BOX → reachable_bbox 剪枝（無 CE 層大檔 walk_roi 秒級）
  → M1–M3.5 已實作（收集 + std_bbox 短路 + repetition 裁剪 + 測試；格式經 KLayout 源碼 +
  真檔雙重確認）。**user pending**：剪枝正確但真檔首次 walk 仍久（卡 [F14] mega-cell），
  暫不續推。見 `docs/plans/F13-sbbox-prune.md`
...
- [F14] placement-heavy mega-cell 優化（user 暫擱）：單一 cell 含千萬級 placement 時 walk_roi
  首碰需 decode 整顆（~283s）。改 numpy 欄位陣列 decode + walk 向量化 ROI 篩選 + 快取。
  見 `docs/plans/F13-sbbox-prune.md` 末「後續」。
