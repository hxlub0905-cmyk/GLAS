...
- [F6] OAS 讀取與批次 fine-align 加速：mmap-backed OasisStream（ROI 路徑）+ 單一 map 共享
  （去雙重 slurp）+ thread-pool 批次 fine-align（per-thread reader）—
  `glas/core/oasis_streamer.py`、`glas/core/oasis_random.py`、`glas/app/gds_align_tool.py` —
  plan: `docs/plans/F6-readwalk-batch-accel.md`（**plan 待 user 核准後開工**；強調逐 byte／逐數值
  golden-output 等價測試，不改演算法與 §7 不變式）

- [F7] 批次對位工作區：Run all/Results… 動作進入的 Batch 工作區（中央左=結果表/直方圖/散點、
  右=SEM overlay、點列就地換 overlay、頂部「← 回對位」）+ inline 進度/結果 streaming（取代 modal）+
  進度條質感升級（漸層+發光+%）— `glas/app/gds_align_tool.py` —
  plan: `docs/plans/F7-batch-workspace-ui.md`（**已核准、開工中**；純 UI 重新安置+視覺，不動 F6 運算與
  §7 不變式。M3 入口改「動作進入+返回鈕」，不放 View segmented）

- [F8] Batch Align 反應性與加速：修串流 O(N²) 重建（節流刷新）+ 進度條回退扁平 +
  thread-pool→ProcessPool（抽 Qt-free `glas/core/fine_align.py`，子行程自建 reader）—
  `glas/core/fine_align.py`、`glas/app/gds_align_tool.py` —
  plan: `docs/plans/F8-batch-responsiveness.md`（**plan 待 user 核准後開工**；修訂 F6 M3 與 F7 M1/M4，
  計算純函式不改、結果 byte/value 不變、§7 不變式不動；cancel 粒度改單張影像邊界）
