...
- [F13] 讀 KLayout per-cell S_BOUNDING_BOX → reachable_bbox 剪枝（無 CE 層大檔 walk_roi 秒級）
  → M1–M3 已實作（收集 + std_bbox 短路 + 測試；格式經 KLayout 源碼 + 真檔雙重確認），
  **待 user 真檔 GUI 驗收**（M4：開 ROI walk 秒級 + `--debug` 看 sbbox_used 大、
  sbbox_violations=0）。見 `docs/plans/F13-sbbox-prune.md`
