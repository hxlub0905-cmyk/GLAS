...
- [F5] Fine align 診斷 + 工作流：殘差疊圖 + 批次總覽（表/直方圖/散點）+ 中位殘差→δ +
  setup 持久化（切 DID 不重設）+ batch cancel/ETA 修復 + overlay 匯出 —
  `glas/app/gds_align_tool.py` — plan: `docs/plans/F5-finealign-diagnostics.md`
  （6 milestone **程式碼全部完成，待 user 本地 GUI 驗收**；驗收後從 §8 移除）
