...
- [F7] 批次對位工作區：新增「Batch」view-mode（中央左=結果表/直方圖/散點、右=SEM overlay、
  點列就地換 overlay）+ inline 進度/結果 streaming（取代 modal）+ 進度條質感升級（漸層+發光+%）—
  `glas/app/gds_align_tool.py` — plan: `docs/plans/F7-batch-workspace-ui.md`
  （**plan 待 user 核准後開工**；純 UI 重新安置+視覺，不動 F6 運算與 §7 不變式）
