# [F5] Fine align 診斷：殘差疊圖 + 批次總覽 + 殘差分析

> **狀態：** planned（待核准）
> **§8 ID：** [F5]
> **建立：** 2026-05-25（2026-05-25 擴充範圍）
...
**未選（暫不做）：** 信心指標 PSR / peak sharpness、診斷影像 PNG 匯出、常駐結果面板、
...
- `glas/app/gds_align_tool.py`（overlay helper、`TemplatePreviewDialog` 擴充、
  `FineAlignResultsDialog`、批次 worker 回傳狀態、`_on_fa_result/_finished`、Results 按鈕、
  median→δ 接線）
- `tests/`（overlay helper / 建表 / 分箱 / 散點 median / status 判定 純邏輯測試）
