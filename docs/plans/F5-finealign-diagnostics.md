...
- [x] `TemplatePreviewDialog` 由 SEM/GDS/Template 三格擴成含「對位前」（coarse anchor，
...
- [x] 新 `FineAlignResultsDialog`：
  ...
  - **殘差散點圖(C1)**：每張影像 (dx,dy) 散點 + 原點十字 + median 標記，分辨系統性偏移 /
    隨機 / 離群。
- [x] 批次 worker 多回傳狀態碼（讓 C3/C4 有資料）：`_on_fa_result` 收
  `(image_id, dx, dy, score, used_r, status)`；單張路徑也保存 used_r/status。
- [x] `Run all` 完成（`_on_fa_finished`）後彈出；另加 panel/toolbar「Results…」可重開。
- [x] 點表格列 / 散點 → 選取並跳到該影像（重用 `_on_sem_image_selected` / list 選取）。
...
- [x] 套用後提示「δ += (mx,my) nm；建議重跑 Run all 看殘差是否收斂」；不自動清空既有
...
- [x] **命名（#1）**：`FineAlignPanel` 的 `BG grey` caption → `Background GL`；`set_pois` 每列的
  ...
  避免動到匯出 / cache schema。
- [x] **setup 記憶（#2）**：MainWindow 新增 `self._fa_setup`（by layer key：是否 POI / 可見 / 顏色 /
  FG），在 user 改 POI 勾選 / 可見性 / FG 時更新；`pois_changed` 與 `set_pois` 路徑寫回。
- [x] **重套**：`_on_roi_finished` 在 `set_document(doc)` 後，依 `self._fa_setup` 用 layer key 比對
...
- [x] **已完成預覽**：batch 進行中每收一筆 `result` 即更新 `self._refined` + badge（現已如此），
...
- [x] 匯出 worker（避免凍 UI）：逐張讀 SEM → 視勾選輸出 `*_raw.png` / `*_overlay.png` 到選定資料夾，
  並寫 **manifest**（CSV/JSON，含 image_id ↔ raw 檔名 ↔ overlay 檔名 ↔ dx/dy/score/status），
  供 MMH 以 image_id join。沿用既有 alignment schema 欄位命名。
- [x] 驗證：py_compile；manifest 列組裝 + 檔名規則純函式測；實際 PNG 落點 / overlay 對齊待 user。
