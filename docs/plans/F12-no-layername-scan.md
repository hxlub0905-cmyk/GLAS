...
> **狀態：** code done (2026-06-04)，待 user 實機驗收
> **§8 ID：** [F12]
> **建立：** 2026-06-04
...
### M1: core — `enumerate_layers` + bounded 抽樣  [status: done]

把純掃描邏輯抽進 core（Qt-free、可單元測試）。**絕不全檔掃**。

- [x] `oasis_streamer.py` 新增 `enumerate_layers(path, *, shared_buf=None,
      progress_cb=None) -> list[dict]`：
  - **fast-path（不變）**：掃 START→首個 CELL 之間的 `LAYERNAME_GEOM/TEXT`，
    有就回傳（與現行行為 byte-for-byte 等價，秒級）。
- [x] **無 LAYERNAME 時走 bounded 抽樣**（新函式，建議放 `oasis_random.py` 因需用
...
- [x] de-dup `(layer, datatype)`；輸出 `{"layer","datatype","name"}` 與 fast-path 一致，
      供 `LayerPickDialog` 直接吃。
- [x] 驗證：`tests/test_oasis_layer_scan.py`
...
### M2: app — 接線 + 串流進度 + cancel  [status: done]

- [x] `_scan_oas_with_streamer`（`gds_align_tool.py:967`）改呼叫 core
      `enumerate_layers`，progress_cb 內 `q.put(("progress", "Sampled K cells —
      found N layers: 17/101, 6/0 …"))`，讓 user 在抽樣中就看到 layer 浮現、要的有了
      就按 cancel。
- [x] cancel：沿用 `LayerScanWorker` 既有 subprocess terminate（純 Python 迴圈，
      process terminate 即停）；抽樣天然有上限不會 hang。
- [x] `_on_scan_finished`（:673）路徑不變（已能開 `LayerPickDialog`）。空結果文案改：
      無 offset 表 → 「請先用 KLayout 另存補索引」；有 offset 但抽樣無幾何 → 提示可手 key。
- [x] 驗證：`QT_QPA_PLATFORM=offscreen pytest tests/test_gds_align*.py` 綠。
- [ ] 手動（待 user 實機）：開無 LAYERNAME 檔 → Scan → 數字 layer 列出 → 勾選 → 帶入 edit。

### M3: 掃描結果 sidecar 快取  [status: done]（user 確認要做）

> 「很多檔」→ 每次重開 app 重抽樣很煩。把列舉結果快取，2 次後即時。

- [x] 列舉結果以 `(path, st_mtime, st_size)` 為 key 存 sidecar（JSON），命中即跳抽樣。
      存放位置沿用 `gds_layer_cache` 的 cache-dir 慣例（檔案可能在唯讀網路碟）。
- [x] 驗證：第二次 scan 命中快取、不進抽樣迴圈（progress_cb 0 次）；mtime/size 變則失效重掃。

### M4: 文件  [status: done]

- [x] README scan 章節補「無 LAYERNAME 檔的數字 layer 列舉」。
- [x] CLAUDE.md §8 把 F12 從撤案改列「進行中 / 完成」；§4 模組說明微調。
- [x] SESSION_LOG.md 新增條目；本 plan 勾 checkbox。
...
- [x] 所有 milestone code/test checkbox 已勾
- [x] `pytest tests/test_oasis_layer_scan.py -v` 通過 + 全套件不退步（574 passed）
- [ ] 手動（待 user 實機）：開一顆無 LAYERNAME 的真實檔 → Scan layers → 數字 layer 正確列出並可勾選載入
