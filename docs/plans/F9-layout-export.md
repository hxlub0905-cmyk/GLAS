...
- [ ] 寫出最小合規 record 序列：MAGIC bytes → START（含 `unit`、offset_flag=0）→ CELLNAME/CELL →
      幾何 record → END（**validation scheme = 0**）。
- [ ] 幾何 record 分支（Q4）：**axis-aligned 矩形偵測 → RECTANGLE record（info-byte + layer/dt + 寬,高 + x,y）**；
      非矩形 → POLYGON record（info-byte + layer/dt + point-list + x,y）。RECTANGLE 與 POLYGON encoder 各一。
...
- [ ] `tests/test_oasis_writer.py`：encode 原語 round-trip、writer→reader 幾何 round-trip、多 layer、
      **RECTANGLE 偵測（矩形→RECTANGLE、非矩形→POLYGON）**、ROI 裁剪頂點、整張(no-crop)輸出、空幾何。
