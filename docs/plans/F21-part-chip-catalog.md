...
### M2: 右欄重構 — PART/CHIP 下拉 + FOV badge [status: done 2026-06-05]
...
- [x] 改寫 `CoordinateSetupPanel` → `PartChipPanel`（保留 `coord_setup` 引用名以
      免動 MainWindow 既有 signal wiring，類別名與 emit 的 dict key 改成新 schema）
- [x] PART/CHIP 下拉：catalog 為空時 disabled + 提示「無 catalog 資料 — 請聯絡管理員」
- [x] 選 CHIP 後即時更新 chip-corner / FOV badge，並 emit `changed`
- [x] Custom FOV collapse 區：勾「Custom」展開兩個 nm spinbox + scale auto checkbox；
      勾掉回 catalog 預設
- [x] 右欄整體佈局：`Load SEM… → PART/CHIP block → Alignment δ block → image list →
      Set/Clear Offset → Load GDS ROI → Fine Align（保留）`
- [x] 修掉所有 `self._fine_dx`/`_fine_dy` 引用（@5003-5004、5423-5424、5545-5546、
      5560-5561、5583-5584、5606-5607、5723-5724、6068-6069），改成只用 `_origin_dx/dy`
- [x] 移除 CoordinateSetupPanel 原 ①/②/③/⑤ 區塊建構碼
- [x] 驗證：`python main.py` 手動跑——選 PART/CHIP 後 SEM jump 落點與舊版手填同數值
      時一致；Custom FOV 可覆蓋預設並反映到 SEM viewer FOV 框

### M3: Alignment δ 常駐區塊 [status: done 2026-06-05]

把 origin δ 從 read-only QLabel 升級為視覺主元素。

- [x] 新增 `AlignmentDeltaPanel` widget：
...
- [x] 接 MainWindow 的 `_origin_dx/dy` 讀取（既有 signal 不動，只換顯示元件）
- [x] 右欄佈局：PART/CHIP block 下方緊接 AlignmentDeltaPanel（兩者都常駐、不收合）
- [x] 原 status-bar 的「origin δ nudged to (...)」訊息仍保留，不衝突
- [x] 驗證：拖 GDS → Set Offset → 數值即時更新；Clear → 歸零；Ctrl+方向鍵 nudge
      數值會走動；點 copy icon → clipboard 內容正確

### M4: Catalog editor（dev mode）[status: done 2026-06-05]

開發者模式下提供 catalog 編輯 UI，工程師預先 key 好 PART/CHIP。

- [x] 新增 `CatalogEditorDialog(QDialog)`：
  ...
  - `Reload` 按鈕：放棄修改、重讀
- [x] 右欄 PART/CHIP block 下方加 `⚙ Edit catalog…` 按鈕（**只在 dev mode 顯示**，
      用 `self._dev_mode` 控制）
- [x] 編輯完按 Save 後，PartChipPanel 重新 `load_catalog` 並刷新下拉
- [x] 驗證：dev mode 開 → 新增 PART/CHIP → Save → 關 dialog → PART 下拉看到新項目；
      重啟 app 仍存在；非 dev mode 看不到此按鈕

### M5: Cache schema bump + part_id/chip_id 追溯 [status: done 2026-06-05]

cache 多存 PART/CHIP id 作追溯，舊 cache 仍可載入（缺 id 就顯示「(legacy)」）。

- [x] `glas/core/gds_layer_cache.py`：`LayerCacheMeta` 加 `part_id: Optional[str]` /
      `chip_id: Optional[str]` 欄位，schema version bump（`v1` → `v2`，loader 容忍 v1
      = 舊欄位都 None）
- [x] 移除 `fine_dx`/`fine_dy` 欄位（讀 v1 cache 時略過、寫 v2 時不寫）
- [x] 載 v2 cache 時：若 `part_id`/`chip_id` 在當前 catalog 內 → 自動選下拉；不在
      → 下拉空白 + status bar 提示「cache from PART X / CHIP Y, not in current catalog —
      showing snapshot values only」（仍能用，只是下拉空）
- [x] 載 v1 cache：跳過下拉、顯示 PART/CHIP「(legacy)」徽章，仍可用快照值
- [x] 驗證：新增 `tests/test_gds_layer_cache.py` 加 v1→v2 round-trip、缺 id 容錯、
      `pytest tests/test_gds_layer_cache.py -v` 全綠

### M6: 收尾 — 文件、§8、SESSION_LOG [status: done 2026-06-05]

- [x] 更新 `README.md`：把使用流程裡的「Coordinate Setup」段改成「Select PART/CHIP」
- [x] 更新 `CLAUDE.md` §5.2「對位流程」描述
- [x] 更新 §7 不要碰的地方：移除 fine tune 相關不變式
- [x] 從 `CLAUDE.md` §8 移除 [F21]
- [x] `SESSION_LOG.md` 加完成條目
- [x] 全套件 `pytest tests/ -v` 通過（預期 ~610+ 項）
