...
### M1: 版面擠迫 / Coordinate Setup 裁切修正  [status: planned]

- [ ] `CoordinateSetupPanel` 展開時欄位/按鈕被裁切：確認 `SemPanel` QScrollArea
  （`:3023`）能正確垂直捲動展開後內容；修正 `CollapsibleSection` body 的
  size policy / scroll content 高度回報，使展開內容不被下方 list 擠掉。
- [ ] 對話框最小寬（LayerFilterDialog 540 `:481`、ExpressionLayerDialog 420 `:1544`、
  AlignmentExportDialog 360 `:3214`）改為「不超過螢幕可用寬」的安全值，避免小視窗被撐破。
- [ ] `MainWindow` 設合理 `setMinimumSize`（≥ 左260+右300+中心最小），避免縮到破版。
- [ ] 整體間距/density 微調，消除「擠在一起」觀感（spacing/margins）。
- [ ] 驗證：py_compile；offscreen 多種視窗尺寸 render-grab 煙霧測試（若 Qt 可用）；
  user 本地拖放縮放確認。

### M2: Layer 列顯示 LAYERNAME 名稱 + 數字  [status: planned]

- [ ] core：開檔掃描時收集 LAYERNAME map `{(layer,datatype) or interval → name}`，
  經 `oasis_random` / 載入路徑帶到 app（streamer 已解析 record 11/12）。
- [ ] `LayerKey` 增 `name`（raw 也可填）或在 `LayerEntry` 帶 display name；
  `_LayerRow` label 改 `NAME (L17/D0)  ·  {n}`，無名稱退回 `L17/D0  ·  {n}`。
- [ ] 驗證：對 LAYERNAME map 解析寫 1–2 個純函式單元測試；label 格式測試。

### M3: 多 POI fine align 核心（合成樣板）  [status: planned]

- [ ] `LayerPanel`：POI 由「互斥單選」改「多選」；`poi_changed` 改帶 list 或新增
  `pois_changed(list)`；移除 `_on_poi_toggled` 的互斥清除邏輯。
- [ ] 資料模型：per-POI FG gray 存哪（`LayerEntry.poi_fg` 或 MainWindow dict
  `{layer_key: fg}`）。
- [ ] 新增 `render_composite_template(poi_specs, anchor, W, H, nm_per_px, bg, blur)`
  純函式：依序把各 POI mask 以各自 FG gray 疊到共用 BG 背景，最後一次 blur。
  保留 `render_poi_template` 作 n=1 包裝以免破壞既有測試。
- [ ] `_on_run_fine_align` / `FineAlignAllWorker` / `poi_polys_for_roi` /
  `_poi_spec` 改吃多 POI（list of spec + 各自 fg）。
- [ ] 匯出：`poi_layer` 欄位改成多層名稱串接（schema 不變）。
- [ ] 驗證：`render_composite_template` 多層疊圖純函式測試（n=1 等同舊行為、
  n≥2 各層灰階正確）；`fine_align_one` 不變沿用既有測試。

### M4: POI 鈕明顯化 + 多選 UI  [status: planned]

- [ ] `_LayerRow` POI 鈕：放大、文字「POI」、加 QSS（未選＝可見輪廓、
  選中＝橘底高亮），tooltip 更新為多選語意。
- [ ] `FineAlignPanel`：POI 區改成「已選 POI 清單」，每列顯示層名＋FG gray 輸入框；
  `set_poi` 改 `set_pois(list)`；BG/blur/radius/threshold 保持全局單組。
- [ ] `_on_poi_changed` 改接 list；Fine Align section 在有 ≥1 POI 時展開。
- [ ] 驗證：多選 toggle 行為、FineAlignPanel `values()` 帶 per-POI fg 結構的測試。

### M5: 合成樣板視覺化彈窗  [status: planned]

- [ ] 新增 `TemplatePreviewDialog`：並排 SEM / GDS overlay / 合成 Template 三圖
  （QLabel+QPixmap 或小 canvas），標題顯示各 POI 名稱與 FG gray、score。
- [ ] `FineAlignPanel` 加「Preview template…」按鈕；MainWindow 組三圖資料後彈窗。
- [ ] 驗證：彈窗以現有 SEM＋合成 template 開得起來（user 本地）；py_compile。
